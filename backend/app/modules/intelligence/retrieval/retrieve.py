...
from app.modules.intelligence.query.parsing import prepare_lexical_text
from app.modules.intelligence.retrieval.deps import RetrievalDeps
from app.modules.intelligence.retrieval.fusion import rank_only
from app.modules.intelligence.schemas.types import RankedList, RetrievalSource

logger = get_logger(__name__)


async def retrieve_semantic(state: SimilarityState, deps: RetrievalDeps) -> dict[str, object]:
    """Vector search in the tenant's own Pinecone namespace.
    ...
    return {
        "rankings": [ranked],
        # Kept for display only -- see `vector_scores` in graphs/state.py for
        # why the score is preserved here while fusion still uses rank alone.
        "vector_scores": {hit.ticket_id: hit.score for hit in hits},
        "trace": [f"retrieve:semantic:{len(ranked.ticket_ids)}"],
    }
